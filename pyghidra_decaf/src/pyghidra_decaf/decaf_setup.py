...
try:
    ...
    try:
        # Third Party Libraries
        from pyhidra import ExtensionDetails  # type: ignore[import-untyped]
        from pyhidra import HeadlessPyhidraLauncher as HeadlessLauncher
    except ImportError:
...
def _load_entry_points(group: str) -> List[TEntryCallable]:
    ...
    try:
        ...
        if hasattr(metadata_entry_points, 'select'):
            ...
            entries = metadata_entry_points.get(group, None)
            if entries is None:
                return []
    ...
    try:
        ...
        for plugin_setup in plugin_setups:
            ...
            logger.debug(
                ...
                details.plugin_version, ext_info.name, any_stub_changed,
            )
